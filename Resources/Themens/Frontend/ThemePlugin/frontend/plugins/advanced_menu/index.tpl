...
{function name="categories_top" level=0}
    ...
    <ul class="menu--list menu--level-{$level} columns--{$columnCount} {if !$hasSub}no-sub-menu--list{/if}"{if $level === 0} style="width: {$menuSizePercentage}%;"{/if}>
            ...
            {foreach $categories as $category}
                ...
                <li class="menu--list-item item--level-{$level}"{if $level === 0} style="width: 100%"{/if}>
                    {block name="frontend_plugins_advanced_menu_list_item"}
                        <a href="{$categoryLink|escapeHtml}" class="menu--list-item-link {if $category.sub && $level == 0}all-caps{/if}" title="{$category.name|escape}"{if $category.external && $category.externalTarget} target="{$category.externalTarget}"{/if}>{$category.name}</a>
